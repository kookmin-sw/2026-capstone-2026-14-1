...
- 결과는 필요 시 기존 `session_event.payload`에 캐시한다.
...
- JSON 입력 기반 한국어 리포트 생성
        ↓
[선택적 저장]
session_event.type = 'AI_HISTORY_REPORT'
        ↓
[프론트 렌더링]
AI 성장 리포트 + 오늘의 운동 미션
```
...
AI 성장 리포트 결과는 필요할 경우 기존 `session_event`에 저장한다.
...
```text
요청마다 기존 DB에서 히스토리 조회
→ HistoryTrendFeature 생성
→ LLM 또는 fallback 생성
→ 응답만 반환
```
...
단점:
- 같은 리포트를 다시 열 때 LLM 비용이 재발생할 수 있음

#### 옵션 B: session_event 캐시

```text
기존 AI_HISTORY_REPORT 이벤트 조회
→ 있으면 캐시 반환
→ 없으면 새로 생성 후 session_event에 저장
```
...
단점:
- session_event가 분석 산출물 저장 용도로도 사용됨
...
```text
1. 현재 로그인 사용자 확인
2. 동일 조건의 최신 AI_HISTORY_REPORT 캐시 조회
3. 캐시가 유효하면 반환
4. 캐시가 없거나 만료되었으면 기존 DB에서 히스토리 조회
5. HistoryTrendFeature 생성
6. confidence gate 통과 여부 확인
7. LLM 또는 fallback으로 리포트 생성
8. session_event에 선택적으로 캐시 저장
9. 결과 반환
```
...
기존 캐시가 있어도 새 리포트를 생성한다. 생성된 결과는 최신 `AI_HISTORY_REPORT` 이벤트로 저장한다.
...
### 4순위: session_event 캐시

`AI_HISTORY_REPORT` 이벤트를 저장하고 재사용한다.
...
11. 새 DB 테이블 없이 기존 DB와 `session_event.payload`만으로 동작한다.
